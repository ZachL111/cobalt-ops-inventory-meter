...
`cobalt-ops-inventory-meter` is centered on automation work in JavaScript. The technical target is: Develop a JavaScript command-oriented project for inventory scenarios with append-only fixtures, checkpoint recovery checks, and no production deployment claims.
...
The score is deterministic. The current threshold is 165. Risk has a penalty of 6, latency has a penalty of 2, and weight contributes a bonus of 4.
